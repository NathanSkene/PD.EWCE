...
```{r }
load("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/CellTypeData_VentralMidbrain10x.rda")
```
...
#biocLite("affy")
library(affy)
data_u133a <- ReadAffy(filenames=u133a_files,celfile.path="/Users/natske/Box Sync/Parkinsons_SGN2017/Data/GSE8397_Raw",compress=TRUE) # Read in the CEL files then normalize
eset_u133a <- rma(data_u133a)
all_dat_u133a = exprs(eset_u133a)
data_u133b <- ReadAffy(filenames=u133b_files,celfile.path="/Users/natske/Box Sync/Parkinsons_SGN2017/Data/GSE8397_Raw",compress=TRUE) # Read in the CEL files then normalize
...
for(reg in unique(region)){
    ...
    write.csv(tt_PD,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Moran/dx_%s.csv",reg))
    tt_Gender = getTT(coef="annot$genderM",eb=eb,probe_annot=annot2)
    write.csv(tt_Gender,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Moran/genderMale_%s.csv",reg))
    ...
    #dendroorder = read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderSten.txt",header = FALSE,stringsAsFactors = FALSE)[,1]
    #plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/dx_%s_lvl2.pdf",reg))
    ...
    dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
    plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/dx_%s_lvl1.pdf",reg))
    res_moran = tt_results$joint_results
    res_moran$list = sprintf("Moran et al \\(2006\\)\n%s",reg)
    save(res_moran,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/Moran_%s.rda",reg))
    ...
    #dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
    #plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/gender_%s_lvl1.pdf",reg))
}
```
...
### LOAD THE EXPRESSION DATA ######
data <- ReadAffy(celfile.path="/Users/natske/Box Sync/Parkinsons_SGN2017/Data/GSE7621_RAW",compress=TRUE) # Read in the CEL files then normalize
eset <- rma(data)
all_data = exprs(eset)
#colnames(all_dat) = newTitle
...
write.csv(tt_PD,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Lesnick/dx.csv"))
tt_Gender = getTT_lesnick(coef="lesnick_annot$gendermale",eb=eb,probe_annot=annot2)
write.csv(tt_Gender,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Lesnick/genderMale.csv"))

## Run EWCE
colnames(tt_PD)[colnames(tt_PD)=="HGNC_Symbol"] = "HGNC.symbol"
tt_results = ewce_expression_data(sct_data=ctd,tt=tt_PD,annotLevel=2,ttSpecies="human",sctSpecies="mouse",reps=10000,thresh=500)
dendroorder = read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderSten.txt",header = FALSE,stringsAsFactors = FALSE)[,1]
plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Lesnick/dx_%s_lvl2.pdf",reg),width=15)
...
save(res_lesnick,file="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Lesnick/Lesnick.rda")
dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
plotEWCE(joint_results=tt_results_lesnick$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Lesnick/dx_%s_lvl1.pdf",reg))

# Run analysis for gender
colnames(tt_Gender)[colnames(tt_Gender)=="HGNC_Symbol"] = "HGNC.symbol"
tt_results = ewce_expression_data(sct_data=ctd,tt=tt_Gender,annotLevel=1,ttSpecies="human",sctSpecies="mouse",reps=10000,thresh=500)
dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Lesnick/gender_%s_lvl1.pdf",reg))
```
...
data <- ReadAffy(celfile.path="/Users/natske/Datasets that are too large to store elsewhere/Disease Transcriptome Datasets/Parkinsons/GSE49036_RAW") # Read in the CEL files
eset <- rma(data)
...
gender = all_dat["224588_at",]<8 # There is a clear seperation between genders based on this... it's not otherwise included
filenames = gsub(".*/","",as.character(pData(phenoData(gse[[1]]))$supplementary_file))
...
ggplot(tsneData)+geom_point(aes(x=tsne1,y=tsne2,col=rin))
...
dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
plotEWCE(joint_results=tt_results_12$joint_results,labelOrders=dendroorder,fName="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/dx_Braak12_lvl1.pdf",height=7)
...
plotEWCE(joint_results=tt_results_34$joint_results,labelOrders=dendroorder,fName="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/dx_Braak34_lvl1.pdf",height=7)
...
plotEWCE(joint_results=tt_results_56$joint_results,labelOrders=dendroorder,fName="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/dx_Braak56_lvl1.pdf",height=7)
...
save(res_merged,file="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/VandeBerg_merged.rda")
plotEWCE(joint_results=res_merged,labelOrders=dendroorder,fName="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/dx_merged_lvl1.pdf",height=7)
```
...
```{r }
probeAnnot = read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/GPL6480-9577.txt",stringsAsFactors = FALSE,sep="\t",skip=17)[,c("ID","GENE_SYMBOL")]
...
for(reg in unique(region)){
    ...
    write.csv(tt_PD,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Corradini/dx_%s.csv",reg))
    tt_Gender = getTT_lesnick(coef="corradini_annot$genderMale",eb=eb,probe_annot=probeAnnot)
    write.csv(tt_Gender,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Corradini/genderMale_%s.csv",reg))
    ...
    tt_results = ewce_expression_data(sct_data=ctd,tt=tt_PD,annotLevel=1,ttSpecies="human",sctSpecies="mouse",reps=10000,thresh=500)
    dendroorder = unique(read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/DendroOrderStenDesc.txt",header = FALSE,stringsAsFactors = FALSE,sep="!")[,1])
    plotEWCE(joint_results=tt_results$joint_results,labelOrders=dendroorder,fName=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Corradini/dx_%s_lvl1.pdf",reg))
    res_Corradini = tt_results$joint_results
    res_Corradini$list = sprintf("Corradini et al (2014)\n%s",reg)
    save(res_Corradini,file=sprintf("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Corradini/Corradini_%s.rda",reg))
...
```{r }
load(file="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/VandeBerg/VandeBerg_merged.rda") # res_merged
load(file="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Lesnick/Lesnick.rda") # res_lesnick
load("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/Moran_Medial SNc.rda") # res_moran
res_moran_medial = res_moran
load("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/Moran/Moran_Lateral SNc.rda")
res_moran_lateral = res_moran
allRes = rbind(res_merged,res_lesnick,res_moran_medial,res_moran_lateral)
plotEWCE(joint_results=allRes,labelOrders=dendroorder,fName="/Users/natske/Box Sync/Parkinsons_SGN2017/Results/EWCE/dx_merged_lvl1.pdf",height=7)
```


# Run CIBERSORT to determine which cell types should be included

```{r }
# Get markers from supplementary table 
markers = read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Data/StenMarkers.csv",stringsAsFactors = FALSE)
markers = markers[markers$X=="Marker",][,-2]
markers = markers[markers$Cluster.ID %in% colnames(ctd[[2]]$specificity),]
markers = markers[,-1]
markers = unique(unlist(markers))
markers = markers[markers %in% rownames(ctd[[2]]$mean_exp)]
eee = ctd[[2]]$mean_exp[markers,]
write.table(eee,file="/Users/natske/Box Sync/Parkinsons_SGN2017/Data/CIBERSORT_SigMatrix.tab",sep="\t",quote=FALSE)
```
...
load("/Users/natske/Box Sync/CellTypeMappingPaper2019/ProcessedData/ctd_StensBigAdult_JuliansVersion_lvl5only.rda")
ctd[[1]]$mean_exp = ctd[[1]]$specificity
ctd = prepare.quantile.groups(ctd,specificity_species="mouse",numberOfBins=40)


gwas_sumstats_path = "/Users/natske/Box Sync/CellTypeMappingPaper2019/ParkinsonMeta2018_Nalls_23andMe.annotated"


tt = read.csv("/Users/natske/Box Sync/Parkinsons_SGN2017/Results/TopTables/Moran/dx_Medial SNc.csv",stringsAsFactors = FALSE)
tt = tt[order(tt$t,decreasing=TRUE),]
geneset = tt$HGNC_Symbol[1:500]
calculate_geneset_enrichment(geneset,gwas_sumstats_path,analysis_name="pd_postMortem_moran",upstream_kb=35,downstream_kb=10,genome_ref_path,geneset_species="human")

geneset = read.csv("/Users/natske/Google Drive/DiseaseLists/IREs.txt",stringsAsFactors = FALSE)[-1,1]
calculate_geneset_enrichment(geneset,gwas_sumstats_path,analysis_name="pd_IREs",upstream_kb=35,downstream_kb=10,genome_ref_path,geneset_species="human")
```
...
attrib_hum = listAttributes(human)
entrezTable = getBM(attributes=c("entrezgene","refseq_mrna"), mart=human)

mirna = read.table("/Users/natske/Downloads/miRDB_v5.0_prediction_result.txt",stringsAsFactors = FALSE)
mirna = mirna[grep("hsa",mirna[,1]),]
newFile = rep("",length=length(unique(mirna$V1)))
i=0
for(mirnas in unique(mirna$V1)){
  i=i+1
  entrez = entrezTable[entrezTable$refseq_mrna %in% mirna[mirna$V1==mirnas,]$V2,]$entrezgene
  newFile[i] = paste(c(mirnas,entrez),collapse=" ")
}
geneCovarFile="/Users/natske/Box Sync/CellTypeMappingPaper2019/miRDB_MAGMA_entrez.txt"
write.table(newFile,file=geneCovarFile,quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)

magmaPaths = get.magma.paths(gwas_sumstats_path,upstream_kb=35,downstream_kb=10)
magma_cmd = sprintf("magma --gene-results '%s.genes.raw' --set-annot '%s' --out '%s.%s'",magmaPaths$filePathPrefix,geneCovarFile,magmaPaths$filePathPrefix,analysis_name="miRDB")
print(magma_cmd)
system(magma_cmd)

    #path = sprintf("%s.%s.sets.out",magmaPaths$filePathPrefix,analysis_name)
    path = sprintf("%s.%s.gsa.out",magmaPaths$filePathPrefix,analysis_name="miRDB")
